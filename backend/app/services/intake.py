...
def score_intake(db: Session, answers: dict[int, int], *, user_id: int | None = None,
    ...
    # тексты интерпретаций
    def interp(code: str, slot: str) -> str:
        row = db.execute(
            select(m.IntakeInterp).where(m.IntakeInterp.direction_code == code,
                                         m.IntakeInterp.slot == slot)
        ).scalar_one_or_none()
        return row.client_text if row else ""

    static = {t.key: t.text for t in db.execute(select(m.IntakeStaticText)).scalars().all()}

    parts = [static.get("pre_result", "")]
    if is_soft:
        parts.append(cfg.soft_no_leading)
    ...
    if leading_dir.module_code:
        ...
        leading_module_name = mod.name if mod else None
    ...
    return {
        ...
        "leading_name": leading_dir.name_leading,       # имя направления (интерпретация)
