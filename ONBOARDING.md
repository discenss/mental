...
День проходится в **две сессии**: 🌅 утро (открыть: фокус + задание) и
🌇 вечер (закрыть: статус задания + квиз + рефлексия → следующий день).
День 7 → недельные итоги: блок из 5+5 маркеров, затем самопроверка (10 вопросов, скрытые
веса) → зона GREEN/YELLOW/RED + critical-logic. Маркеры спрашиваются раз в неделю, а не
каждый день (ежедневные 10 вопросов утомляли и не давали новой информации).
