...
ДЕНЬ  = утро:  Фокус дня
        ...
        вечер: 3 вопроса рефлексии

ДЕНЬ 7 = итоговый фокус → итоговое задание → AUDIO FINAL → квиз/фиксация
         → 3 рефлексии → 5+5 маркеров недели → недельная самопроверка(10)
...
daily_entries                -- один ряд на пользователя на день модуля
  id, enrollment_id, week_n, day_n, date,
    morning_answers JSON,    -- ответы 5 «утренних» маркеров недели (пишутся в день 7)
    task_status (DONE|PARTIAL|NOT_DONE),
    task_answer JSON?,       -- ответы задания (в дневник по желанию)
    quiz_answer,
    evening_answers JSON,    -- ответы 5 «вечерних» маркеров недели (пишутся в день 7)
...
- ровно 6 недель × 7 дней; 5+5 маркеров (спрашиваются раз в неделю, перед самопроверкой);
  3 вопроса рефлексии/день; 10 вопросов самопроверки/неделя;
...
**НЕ сохраняется:** недельные маркеры, техотметка выполнения, квизы, самопроверка и её баллы.
...
GET  /api/v1/today                             — текущий день: фокус, задание, аудио, квиз
POST /api/v1/day/morning                       — открыть день
POST /api/v1/day/task-status                   — DONE|PARTIAL|NOT_DONE (+ task_answer в дневник)
POST /api/v1/day/quiz                           — ответ квиза
POST /api/v1/day/evening                       — 3 рефлексии
POST /api/v1/week/selfcheck                    — 5+5 маркеров недели + 10 ответов
                                                 → {zone, user_text, recommendation, criticals}
